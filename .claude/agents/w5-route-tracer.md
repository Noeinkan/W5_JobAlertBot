...
- A frontend fetch URL string (look for it in `src/dashboard/public/js/*.js`)
...
5. **Find the frontend caller.** `Grep` for the path across `src/dashboard/public/js/*.js` (the frontend was split into `app-core`, `app-charts`, `app-filters`, `app-table`, `app-render`, `app-explorer`, `app-bootstrap`; fetch call sites mostly live in `app-bootstrap.js`). Capture the fetch call site.
